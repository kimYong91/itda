...
@Dao
interface MealDao {
    @Query("SELECT * FROM itdafood WHERE strftime('%Y-%m-%d', date / 1000, 'unixepoch') = strftime('%Y-%m-%d', :date / 1000, 'unixepoch') " +
    ...
    @Query("SELECT * FROM itdafood")
    fun getAllMeals(): LiveData<List<Meal>> // LiveData를 반환하도록 수정

    @Query("SELECT * FROM itdafood WHERE date BETWEEN :startDate AND :endDate")
